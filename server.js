...
function logRecaptchaFailure({
    ...
    const normalizedRequestId = String(requestId || 'n/a');
    const normalizedCode = String(code || 'recaptcha_failed');
    ...
    const normalizedProviderCodes = Array.isArray(providerCodes)
        ? providerCodes.map(item => String(item || '').trim()).filter(Boolean)
        : [];
...
async function verifyRecaptcha(token, request, expectedAction = '') {
    ...
    const secret = String(process.env.RECAPTCHA_SECRET || RECAPTCHA_SECRET || '').trim();
    ...
    try {
        ...
        const networkMessage = error?.name === 'AbortError'
            ...
            : String(error?.message || 'network_error');
        logRecaptchaFailure({
            requestId,
...
function validateCsrf(request, response, next) {
    ...
    if (!sessionId || !expectedToken || !providedToken || expectedToken !== providedToken) {
        return response.status(403).json({ ok: false, error: 'Token CSRF inválido o ausente' });
    }
...
app.post('/api/mp/create-preference', async (req, res, next) => {
    try {
        ...
        const successUrl = `${BASE_URL}/success.html?order_id=${encodeURIComponent(orderId)}&order_ref=${encodeURIComponent(externalReference)}`;
        const failureUrl = `${BASE_URL}/failure.html?order_ref=${encodeURIComponent(externalReference)}`;
        const pendingUrl = `${BASE_URL}/pending.html?order_ref=${encodeURIComponent(externalReference)}`;
        const nowIso = new Date().toISOString();
    ...
    try {
        ...
        if (paymentMethod === PAYMENT_METHODS.MERCADOPAGO && depositAmount > 0) {
            ...
            const preferenceData = {
                ...
                back_urls: {
                    success: `${BASE_URL}/pending.html?order_ref=${encodeURIComponent(externalReference)}&quote_id=${encodeURIComponent(quoteId)}`,
                    failure: `${BASE_URL}/failure.html?order_ref=${encodeURIComponent(externalReference)}&quote_id=${encodeURIComponent(quoteId)}`,
                    pending: `${BASE_URL}/pending.html?order_ref=${encodeURIComponent(externalReference)}&quote_id=${encodeURIComponent(quoteId)}`
                }
            };
...
if (!isProduction) {
    app.get('/debug/recaptcha', (req, res) => {
        res.json({
            ok: true,
            requestId: req.requestId,
            recaptchaSecretConfigured: Boolean(String(process.env.RECAPTCHA_SECRET || '').trim()),
